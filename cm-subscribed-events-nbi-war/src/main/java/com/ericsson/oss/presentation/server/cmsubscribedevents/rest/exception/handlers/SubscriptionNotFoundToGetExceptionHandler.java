...
/**
 * This Exception handler will handle any SubscriptionNotFoundToGetException thrown and return a Response Object with the error messages in the response
 * object body in JSON format. When JAX-RS sees that a SubscriptionNotFoundToGetException has been thrown by application code, it catches the exception and calls its
 * getResponse() method to obtain a Response to send back to the client.These classes need to be registered in web.xml.
 */
@Provider
public class SubscriptionNotFoundToGetExceptionHandler implements ExceptionMapper<SubscriptionNotFoundToGetException> {
    ...
    @Override
    public Response toResponse(final SubscriptionNotFoundToGetException exception) {
