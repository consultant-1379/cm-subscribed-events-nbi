...
/**
 * This Exception handler will handle any SubscriptionNotFoundToDeleteException thrown and return a Response Object with the error messages in the response
 * object body in JSON format. When JAX-RS sees that a SubscriptionNotFoundToDeleteException has been thrown by application code, it catches the exception and calls its
 * getResponse() method to obtain a Response to send back to the client.These classes need to be registered in web.xml.
 */
@Provider
public class SubscriptionNotFoundToDeleteExceptionHandler implements ExceptionMapper<SubscriptionNotFoundToDeleteException> {
    ...
    @Override
    public Response toResponse(final SubscriptionNotFoundToDeleteException exception) {
        final String responseBody = ERROR_JSON_FORMAT.format(new Object[] { RESOURCE_COULD_NOT_BE_FOUND });
        systemRecorder.recordCommand("DELETE", CommandPhase.FINISHED_WITH_ERROR, CM_SUBSCRIBED_EVENTS_NBI, SUBSCRIPTIONS, RESOURCE_COULD_NOT_BE_FOUND);
